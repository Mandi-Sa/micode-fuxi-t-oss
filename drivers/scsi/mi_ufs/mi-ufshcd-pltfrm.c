			 ...
			 struct ufs_vreg **out_vreg)
{
	...
	if (strlen(name) && !strncmp(name, "vcc", strlen(name))) {
		u32 value = 0; //mv
		/* need check vcc range in device tree */
		if (ebuff_value_u32("vcc", &value)) {
			dev_info(dev, "%s: ebuff origin min_uV %d value %dmV\n", __func__, vreg->min_uV, value);
			vreg->min_uV = value * 1000;
			vreg->max_uV = value * 1000;
		}
	}
#endif
...
#ifdef CONFIG_UFS_EBUFF
	if (ebuff_value_u32("lanes_per_dir", &value) && (value > 0 && value <= UFSHCD_DEFAULT_LANES_PER_DIRECTION)) {
		dev_info(hba->dev, "%s: ebuff origin lanes-per-direction 0x%x value 0x%x\n", __func__, hba->lanes_per_direction, value);
		hba->lanes_per_direction = value;
	}
#endif
}
