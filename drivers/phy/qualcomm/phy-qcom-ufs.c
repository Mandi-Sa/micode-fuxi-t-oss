...
bool ebuffphyha_value_u32(char *pname, u32 *value)
{
	...
	for (i = 0; i < sizeof(ebuffphyha_data) / sizeof(struct ebuffphyha); i++) {
		if (ebuffphyha_data[i].enable_flag && !strncmp(ebuffphyha_data[i].pname, ptr, sizeof(ebuffphyha_data[i].pname))) {
...
bool of_obtain_ebuffphyha_info(void)
{
	...
	if (!ebuff_node)
		return false;
	for (i = 0; i < sizeof(ebuffphyha_data) / sizeof(struct ebuffphyha); i++) {
		ret = of_property_read_string(ebuff_node, ebuffphyha_data[i].name, (const char **)&buff);
		if (ret || !buff)
			continue;
