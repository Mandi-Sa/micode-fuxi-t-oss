...
#define QUERY_ATTR_IDN_CLD_LEVEL 0x34
#define QUERY_ATTR_IDN_DEFRAG 0x35
...
{
	struct ufs_hba *hba = cld->hba;
	bool kio_trigger = trigger?1:0;

	return mi_ufs_query_flag(hba, UPIU_QUERY_OPCODE_SET_FLAG, (enum flag_idn)QUERY_ATTR_IDN_CLD_ENABLE, 0, 0, &kio_trigger);
}
